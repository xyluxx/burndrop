...
- `libsodium-wrappers-sumo` 0.8.x for `crypto_box_seal`, `crypto_aead_xchacha20poly1305_ietf_decrypt`, `crypto_pwhash` (Argon2id, for password-protected reveals), `crypto_generichash`, `sodium_pad`, `randombytes_buf`, and `memzero`. Pinned by version and by hash of the bundled WebAssembly. The sumo build is required because the standard build has no `crypto_pwhash`. Verified facts (2026-09-17): neither build exposes `crypto_hash_sha256` in a way worth a second code path, so fingerprints and commitments in the browser use WebCrypto `crypto.subtle.digest("SHA-256")`, which every browser has. The WebAssembly build requires `'wasm-unsafe-eval'` in `script-src` (tested in Chromium; without it `WebAssembly.instantiate` is blocked and the ESM build has no JavaScript fallback). That directive permits WebAssembly compilation only, not script evaluation, and is documented in the crypto spec. Size: about 620 KB minified with the sumo build, about 200 KB gzipped, which is acceptable for a page that loads once per exchange. The fallback if the directive is judged unacceptable is the audited pure-JavaScript `@noble` family, at the cost of composing the sealed box from primitives ourselves.
...
- TypeScript: `libsodium-wrappers-sumo`, native `fetch`, WebCrypto SHA-256, `@napi-rs/keyring` for the OS keychain backend. `engines.node >= 22.13` (Node 20 reached end of life in April 2026).
...
- **Page copy reduced and split layout (2026-09-17).** The drop page shows the title, two labeled rows (Why: the purpose sentence; Link expires: the countdown), the field, and the button. The reveal page shows What (the secret name) and Link expires. The name on the drop page, storage, retention, the key fingerprint, and the reveal flags sit under a collapsed Details line instead of a metadata panel, and the explanatory paragraphs are gone. The agent instructions now tell the model to write the purpose for the page and to pick the ttl. From 1024 px up the page is a two-panel layout like a sign-in screen: the form column on the left, a brand panel (glyph, wordmark, tagline) on the right. Below that it is a single column, edge to edge on phones, with the theme toggle fixed bottom right.
- **Optional reveal password (2026-09-17).** The human can require a password on every reveal link. It is set in a terminal (`burndrop reveal-password set`), kept in the OS credential store, and mixed into the link key with Argon2id and BLAKE2b (crypto specification section 4.1), so neither the link nor the password alone decrypts the value. The agent can only turn the requirement on or off (the `reveal_password` MCP tool) because the MCP specification forbids collecting sensitive data through elicitation and a password typed into the chat would sit in the transcript. The page asks for the password before it opens the slot and lets the human retry a wrong one locally, since the relay copy is gone by then. The page needs the sumo build of libsodium for `crypto_pwhash`, which adds about 100 KB.
- **Delivery guidance and sent-link revocation (2026-09-17).** The link may travel over any channel the agent already uses with its human (chat, email, Telegram, Slack, tickets); none is trusted. Because the model chooses the channel, misdelivery by the agent is the likeliest human-error path, so the instructions gained rule 2 (only to the human you work for, never anywhere shared) and every `request_secret` and `send_secret` result carries a `delivery` field repeating it, so the reminder arrives on every call. To make the mistake revocable, `send_secret` keeps the reveal's revoke token in a `sent` record until the link expires, and `revoke_request` cancels unopened reveal links as well as request links, returning the state that says whether someone got there first.
